//компиляция: nvcc -arch=sm_86 pre_visualization_pocessing128.cu -o pre_visualization_pocessing128 --ptxas-options=-v
#include <iostream>
#include <fstream>
#include <vector>
#include <string>
...
const size_t h_groupOffsets[NUM_GROUPS] = {
    ...
    5,  // UNIFORMITY (столбец 5)
    6,  // BIT_FREQ (столбцы 6-69)
    70, // PAIRS (столбцы 70-73)
    74, // TRIPLES (столбцы 74-81)
    82, // QUADS (столбцы 82-97)
    98, // ENTROPY (столбцы 98-161)
    162, // GLOBAL_STATS (столбец 162)
    163, // RUN_TESTS (столбец 163)
    164, // BLOCK_FREQUENCY (столбцы 164-179)
    180, // MUTUAL_INFORMATION (столбцы 180-182)
    183, // AUTOCORR (столбцы 183-192)
    193, // DIFFERENCES (столбцы 193-448)
    449, // CLUSTERS (столбцы 449-452)
    459  // CLUSTER_COUNT
};
...
__global__ void processData(
    ...
    switch (group) {
        ...
        case CLUSTERS: {
            ...
            int next_cluster_num = (tid - d_groupOffsets[CLUSTERS] + 1) % 10;
...
// Основная программа
int main() {
    std::string inputFilename = "./data/analysis_results128.csv";
    std::string outputFilename = "./data/processed_data128.csv";
