...
# Загрузка данных
file_path = './data/processed_data256.csv'
...
def plot_group(
    ...
    os.makedirs('./picture256', exist_ok=True)
    plt.savefig(f'./picture256/{save_as}')
    plt.show()
...
def plot_differences_with_intervals(data, columns, title, save_as):
    ...
    plt.tight_layout(rect=[0, 0, 1, 0.96])
    os.makedirs('./picture256', exist_ok=True)
    plt.savefig(f'./picture256/{save_as}', bbox_inches='tight')
    plt.show()
...
def plot_clusters_global(data, mean_columns, distance_columns, count_columns, title, xlabel, ylabel_mean, ylabel_distance, ylabel_count, window_title, save_as):
    ...
    plt.gcf().canvas.manager.set_window_title(window_title)
    os.makedirs('./picture256', exist_ok=True)
    plt.savefig(f'./picture256/{save_as}', bbox_inches='tight')  # Учитываем легенду
    plt.show()
...
plot_group_pair(
    data,
    columns_top=[f"Correlation_mean_{i}" for i in range(4)],
    columns_bottom=[f"Correlation_dispersion_{i}" for i in range(4)],
    ...
    legend_labels_top=[f"r (s{i},res)" for i in range(4)],
    legend_labels_bottom=[f"r (s{i},res)" for i in range(4)],
    alpha_num=0.5,
...
plot_group_pair(
    data,
    columns_top=[f"MutualInformation_mean_{i}" for i in range(10)],
    columns_bottom=[f"MutualInformation_dispersion_{i}" for i in range(10)],
    title_top="Взаимная информация между состояниями s0, s1, s2, s3, и result: Средние значения",
    title_bottom="Взаимная информация между состояниями s0, s1, s2, s3, и result: Дисперсии",
    xlabel="Индекс",
    ylabel_top="Средние значения",
    ylabel_bottom="Дисперсии",
    window_title="Взаимная информация между состояниями s0, s1, s2, s3, и result",
    #legend_labels_top=['I (s0;s1)','I (s0;s2)','I (s0;s3)','I (s1;s2)','I (s1;s3)','I (s2;s3)','I (res;s0)','I (res;s1)','I (res;s2)','I (res;s3)'],
    legend_labels_top = [f'I ({x};{y})' for x in ['s0', 's1', 's2', 's3', 'res'] for y in ['s0', 's1', 's2', 's3'] if x < y],
    legend_labels_bottom=['I (s0;s1)','I (s0;s2)','I (s0;s3)','I (s1;s2)','I (s1;s3)','I (s2;s3)','I (res;s0)','I (res;s1)','I (res;s2)','I (res;s3)'],
    alpha_num=0.5,
...
plot_clusters_global(
    data=data,
    mean_columns=[f'Clusters_mean_{i}' for i in range(32)],
    distance_columns=[f'Clusters_distance_{i}_{(i + 1) % 32}' for i in range(32)],
    count_columns=[f'ClusterCount_mean_{i}' for i in range(32)],
...
plot_detailed_clusters(
    data=data,
    mean_columns=[f'Clusters_mean_{i}' for i in range(32)],
    distance_columns=[f'Clusters_distance_{i}_{(i + 1) % 32}' for i in range(31)],
    count_columns=[f'ClusterCount_mean_{i}' for i in range(32)],
