//компиляция: nvcc -arch=sm_86 collect_statistics256_spectral_analysis.cu -o collect_statistics256_spectral_analysis --ptxas-options=-v -lcufft
...
std::vector<uint64_t> readCSV(const std::string& filename) {
    ...
    while (getline(file, line)) {
        ...
        sscanf(line.c_str(), "%lx,%lx,%lx,%lx", &s0, &s1, &s2, &s3);
        states.push_back(s0);
        states.push_back(s1);
        states.push_back(s2);
        states.push_back(s3);
...
__global__ void generateSequence(uint64_t* states, cufftComplex* d_fftBuffer, size_t rows) {
    ...
    if (idx < rows) {
        ...
        uint64_t s2 = states[idx * 2 + 2];
        uint64_t s3 = states[idx * 2 + 3];

        for (int i = 0; i < SEQUENCE_LENGTH; ++i) {
            uint64_t res = (((s1 * 5) << 7) | ((s1 * 5) >> (64 - 7))) * 9;
...
int main() {
    try {
        ...
        std::ofstream resultFile("./data/analysis_results256_spectral.csv");
        ...
        for (size_t offset = 0; offset < totalRows; offset += BLOCK_ROWS) {
            ...
            if (cudaMalloc(&d_states, currentRows * 4 * sizeof(uint64_t)) != cudaSuccess) {
            ...
            cudaMemcpy(d_states, &initialStates[offset * 4], currentRows * 4 * sizeof(uint64_t), cudaMemcpyHostToDevice);
        ...
        resultFile.close();
        std::cout << "Анализ завершён, результаты сохранены в './data/spectral_analysis_results256.csv'." << std::endl;
